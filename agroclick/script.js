...
function processCell(cell){
  if(cell.type==='wheat'){
    ...
    if(mage&&mage.remainingTargets>0){
      ...
      setStatus(`Кот-маг усилил грядку ${cell.id+1}: ${before} → ${markLogValue(amount+' ед.')} (${markLogValue('+10 ед.')}).`,'✨','character',`character:${mage.token}`,{append:true});
...
function processMole(cell){
  ...
  const wheat=shuffle(state.board.filter(c=>c.type==='wheat'&&c.opened&&!c.stolen&&!c.burned&&!c.destroyed&&c.amount>0)).slice(0,3);
  const stolenDetails=[];
  let stolenTotal=0;
  wheat.forEach(w=>{
...
function tickExistingEffects(boardTimerIds,inventoryTimerIds,characterToken){
  ...
  if(state.character && !state.character.inactive && state.character.token===characterToken){
    if(state.character.type==='defender'){
      ...
      state.character.timer--;
      if(state.character.timer<=0){ const before=state.locationHarvest; multiplyHarvest(2); const after=state.locationHarvest; clearCharacter(`Кот-учёный удвоил текущий урожай локации: ${before} → ${markLogValue(after+' ед.')} (${markLogValue('+'+(after-before)+' ед.')}).`); }
...
function tickEconomist(){
  ...
  if(candidates.length){
    ...
    setStatus(`Кот-экономист усилил грядку ${target.id+1}: ${before} → ${markLogValue(target.amount+' ед.')} (${markLogValue('+'+bonus+' ед.')}).`,'📈','character',`character:${ch.token}`,{append:true});
  }
  ch.timer--;
...
function inventoryBonusApplicable(type,view=currentView()){
  ...
  if(type==='mineralFertilizer') return board.some(c=>c.type==='wheat'&&c.opened&&!c.stolen&&!c.burned&&!c.destroyed&&!c.fertilized&&c.amount>0);
...
function hasInventoryBonus(type,view=currentView()){
  return Array.isArray(view.inventory)&&view.inventory.some(item=>item.type===type&&(item.timer==null||item.timer>0));
}
function isResolvedChestBonusActive(type,view=currentView()){
  if(!type) return false;
  if(type==='bombShield'||type==='scarecrow') return hasInventoryBonus(type,view);
  if(type==='digitalMagnifier'||type==='mineralFertilizer'||type==='portableNeutralizer'||type==='goldenHoe'){
    return hasInventoryBonus(type,view)&&inventoryBonusApplicable(type,view);
  }
  return false;
}
...
function useFertilizer(viewIndex=state.viewLocationIndex){
  const view=currentView();
  const candidates=view.board.filter(c=>c.type==='wheat'&&c.opened&&!c.stolen&&!c.burned&&!c.destroyed&&!c.fertilized&&c.amount>0);
  ...
  openInlineAction('🧪 Минеральное удобрение','Выберите подсвеченную пшеничную ячейку для удвоения.',[],id=>{
    withViewedLocationContext(viewIndex,()=>{
      const c=cellById(id);
      if(!c||c.type!=='wheat'||!c.opened||c.stolen||c.burned||c.destroyed||c.fertilized||!(c.amount>0)){ toast('Эта пшеничная ячейка недоступна для удобрения.'); return; }
...
function render(){
  ...
  renderCharacter(view); renderBoard(view); renderInventory(view); renderBeetleIndicator();
  if(state.finished&&state.finalResult&&!state.resultDialogDismissed&&!$('resultDialog').open) showResult();
}

// Показывает постоянное предупреждение о накопленных жуках под блоком «Активы».
// Индикатор использует игровую PNG-иконку и остаётся активным во всех локациях.
function renderBeetleIndicator(){
  const indicator=$('beetleIndicator');
  const countNode=$('beetleIndicatorCount');
  if(!indicator||!countNode) return;
  const count=Math.max(0,Number(state.beetles)||0);
  indicator.classList.toggle('active',count>0);
  indicator.setAttribute('aria-hidden',String(count<=0));
  countNode.textContent=count>1?`×${count}`:'';
}
...
function renderBoard(view=currentView()){
  ...
  $('board').innerHTML=board.map(c=>{
    ...
    if(!rabbit&&(c.opened||c.revealed)){
      ...
      else if(c.type==='chest'&&c.chestResolved) body=`${c.resolvedBonus?`<span class="cell-effect-backdrop chest-effect-backdrop" title="${BONUS_NAMES[c.resolvedBonus]||''}">${assetMarkup(c.resolvedBonus,ICONS[c.resolvedBonus]||'')}</span>`:''}<span class="icon resolved-chest-icon cell-foreground">${assetMarkup('chest','🎁')}</span><span class="resolved-chest-label cell-foreground">Открыт</span>`;
      ...
      else if(c.type==='weed'&&c.summonedBeetle) body=`<span class="cell-effect-backdrop beetle-effect-backdrop" title="Сорняк призвал жука">${assetMarkup('beetle',ICONS.beetle)}</span><span class="icon cell-foreground">${assetMarkup('weed',ICONS.weed)}</span><span class="amount cell-foreground">${typeLabel(c.type)}</span>`;
    ...
    const originBadges=!rabbit?`${c.summonedByMole&&(c.type==='bomb'||c.type==='weed')?`<span class="origin-badge mole-origin" title="Призвано кротом">${assetMarkup('moleSource','🐹')}</span>`:''}`:'';
  ...
  document.querySelectorAll('.resolved-chest-cell .chest-effect-backdrop').forEach(node=>{
    const cellId=Number(node.closest('.cell')?.dataset.cellId);
    const cell=board.find(item=>item.id===cellId);
    const active=isResolvedChestBonusActive(cell?.resolvedBonus,view);
    node.classList.toggle('chest-bonus-active',active);
    node.classList.toggle('chest-bonus-inactive',!active);
  });
...
function resolveStatusAssetKey(row={}){
  if(/^chest:/.test(String(row.groupKey||''))) return 'chest';
