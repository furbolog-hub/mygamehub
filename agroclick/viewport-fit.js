...
(() => {
  ...
  /* --- Точный размер клетки поля: считаем по фактическим размерам панели.
     Клетка = min((ширина - зазоры)/колонки, (высота - зазоры)/строки).
     Поле никогда не перерастает панель (не налезает на журнал),
     клетки всегда квадратные. Игровая логика не затрагивается. --- */
  function fitBoard() {
    var board = document.getElementById('board');
    if (!board) return;
    var panel = board.parentElement;
    if (!panel) return;
    var cs = getComputedStyle(board);
    var bw = parseInt(cs.getPropertyValue('--bw'), 10) || 5;
    var bh = parseInt(cs.getPropertyValue('--bh'), 10) || 6;
    var gap = parseFloat(cs.getPropertyValue('--gap')) || 6;
    var ps = getComputedStyle(panel);
    var pw = panel.clientWidth - (parseFloat(ps.paddingLeft) || 0) - (parseFloat(ps.paddingRight) || 0);
    var ph = panel.clientHeight - (parseFloat(ps.paddingTop) || 0) - (parseFloat(ps.paddingBottom) || 0);
    if (!(pw > 0) || !(ph > 0)) return;
    var byW = (pw - (bw - 1) * gap) / bw;
    var byH = (ph - (bh - 1) * gap) / bh;
  ...
  function watchBoard() {
    ...
    try {
      if (typeof ResizeObserver === 'function') {
        var ro = new ResizeObserver(function () { fitBoard(); });
        ro.observe(board.parentElement);
      }
    } catch (e) {}
