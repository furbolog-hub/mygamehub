...
function floatingGain(text) {
  if (reduceMotion) return;
  const target=$('lakeCard'); if (!target) return;
  const rect=target.getBoundingClientRect();
  const el=document.createElement('div'); el.className='floating-gain'; el.textContent=text;
  el.style.left=`${rect.left+rect.width*.58}px`; el.style.top=`${rect.top+rect.height*.55}px`;
...
function makeFish(category='normal', source='Заброс', direct=true) {
  ...
  if (state.weather==='golden') { const plus=Math.floor(rand(1,5)); f.weight=round1(f.weight+plus); f.tags.push(`<span class="gold-add">+${plus} кг</span>`); floatingGain(`+${plus} кг`); }
...
function renderHistory() {
  ...
  if (lastAnimatedHistoryId) {
    ...
    latest?.scrollIntoView?.({behavior:reduceMotion?'auto':'smooth',block:'nearest'});
