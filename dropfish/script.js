...
function hydrateState(raw) {
  ...
  const restored={...base, ...parsed, rifts:{...base.rifts,...(parsed.rifts||{})}, islands:{...base.islands,...(parsed.islands||{})}, abyssal:{...base.abyssal,...(parsed.abyssal||{})}, mythic:{...base.mythic,...(parsed.mythic||{})}, disabledBonusIds:new Set(parsed.disabledBonusIds || [])};
...
function fishHistoryStatus(h,fish){
  if(!fish)return null;
  if(fish.islandSkeleton||fish.mythicSkeleton||fish.abyssSkeleton)return {file:'skeleton',label:'Рыба превратилась в скелет',kind:'skeleton'};
...
function fishSkeletonMainIcon(fish){
  if(!fish?.islandSkeleton&&!fish?.mythicSkeleton&&!fish?.abyssSkeleton)return '';
...
function renderEssenceImpact(fish) {
  ...
  if (!impacts.length) return '';
  return impacts.map(e=>`<div class="essence-impact">${artifactIconMarkup('Эссенция «Великан Океанов»','epic','essence-icon')}<span><strong>Эссенция «Великан Океанов»</strong>: ${kg(e.before)} → ${kg(e.after)} (×${e.factor})</span></div>`).join('');
}
...
function renderFinalOrcaImpact(fish) {
  ...
  return `<div class="gear-impact final-orca-impact is-eaten"><span class="gear-icon">${icon}</span><span><strong>Касатка съела рыбу после финального усиления</strong><small>Вес достиг ${kg(impact.weight)}<br>В итоговый вес не включена</small></span></div>`;
}
...
function renderDebuffWeightImpact(fish){if(!fish?.debuffLimited||!Number.isFinite(fish.preDebuffWeight)||!Number.isFinite(fish.debuffBaseWeight))return '';const source=fish.debuffSource||'Дебаф';return `<div class="gear-impact fish-debuff-impact"><span class="fish-effect-badge-slot">${fishEffectBadge('weight-limit','Ограничение в весе')}</span><span><strong>${source}</strong><small>${kg(fish.preDebuffWeight)} → <b class="island-weight-final">${kg(fish.debuffBaseWeight)}</b></small></span></div>`;}
...
function renderEmbeddedFishList(row) {
  ...
  const items=row.embeddedFishIds
    ...
    .map(fish=>{
      const eaten=fishIsEaten(fish)||Boolean(fish.abyssLost)||Boolean(fish.riftSacrificeLabel);
      const stolen=fishIsStolen(fish);
       const displaced=Boolean(fish.islandDisplaced),traded=Boolean(fish.islandTraded),skeletonIcon=fishSkeletonMainIcon(fish),eatenIcon=fishEatenMainIcon({eaten,riftSacrificeLabel:fish.riftSacrificeLabel,abyssLost:fish.abyssLost},fish),icon=displaced?'<span class="island-displaced-cross">✖</span>':traded?shipIconMarkup(fish.tradeVessel==='recyclon'?'recyclon':'trade','is-embedded'):skeletonIcon||eatenIcon||fishCategoryIcons(fish,'is-embedded',row.type!=='rift');
      ...
      const colossusClass=fish.islandColossus?` is-island-colossus tier-${fish.rarity||'common'}${row.id===lastAnimatedHistoryId?' is-new-colossus':''}`:fish.islandMoray?` is-island-moray tier-${fish.rarity||'common'}`:fish.islandGraniteCatfish?` is-island-granite-catfish tier-${fish.rarity||'common'}`:'';
       const riftIcon=skeletonIcon||eatenIcon?'':riftFishIcon(fish)||islandFishIconMarkup(fish),riftBadges=riftFishBadges(fish,'',row.type!=='rift')||(riftIcon&&islandFishKind(fish)?islandFishBadges(fish):'');
...
function islandLootLabel(item){
  if(item.kind==='colossus')return `${islandFishIconMarkup(item,'is-loot-icon')} ${item.name} — ${kg(item.weight)}`;
  if(item.kind==='moray')return `${islandFishIconMarkup(item,'is-loot-icon')} ${item.name} — ${kg(item.weight)}`;
  if(item.kind==='graniteCatfish')return `${islandFishIconMarkup(item,'is-loot-icon')} ${item.name} — ${kg(item.weight)}`;
  if(item.kind==='echoRay')return `${islandFishIconMarkup(item,'is-loot-icon')} ${item.name} — ${kg(item.weight)}`;
  if(item.kind==='trade')return `${tradeItemIconMarkup(item)} ${item.name}`;
  if(item.kind==='cache')return `${islandLootIconMarkup(item)} Тайник: ${item.items.map(x=>tradeItemIconMarkup(x)+' '+x.name).join(', ')}`;
  if(item.kind==='flare')return `${islandLootIconMarkup(item)} Руническая сигнальная ракета`;
  ...
  if(item.kind==='firstWaterFlask')return `${islandLootIconMarkup(item)} Флакон Первой Воды`;
  if(item.kind==='mistSupplies'){const fish=item.islandFish?islandLootLabel(item.islandFish):`${item.standardFish.length} стандартные рыбы`;return `${islandLootIconMarkup(item)} Припасы ордена Мглистой Дымки: ${fish}${item.resource?` + ${tradeItemIconMarkup(item.resource)} ${item.resource.name}`:''}`;}
...
function resolveIslandLocationResult(a,loc,outcome,repeated=false){
  ...
  const settleSafe=()=>{if(a.actionsLeft<=0&&a.island==='leadenFog'&&!a.routeMapUsed&&a.loot.some(x=>x.kind==='routeMap')&&a.visited.length===3){a.routeMapUsed=true;a.maxVisits=4;a.actionsLeft=1;message+='<br><strong>🗺️ Карта последнего маршрута открыла четвёртое исследование.</strong>';}a.stage=a.actionsLeft<=0?'finished':'result';a.message=message;renderIsland();commitState();offerIslandMaskRepeat(a,loc);};
...
function markFishDisplaced(fish){fish.removed=true;fish.islandDisplaced=true;state.history.forEach(row=>{if(row.type==='fish'&&row.fishId===fish.id)row.islandDisplaced=true;});}
function commitIslandLoot(a,parentHistoryId=null){
  ...
  a.loot.filter(x=>x.kind==='mistSupplies').forEach(pack=>{pack.standardFish.forEach(category=>{const natural=category==='giant'?rand1(20,40):category==='heavy'?rand1(10,19.9):rand1(.1,9.9),distorted=state.islands.unstablePresence,weight=distorted?rollIslandDistortedWeight(natural):natural,f={id:uid(),name:pick(category==='giant'?DATA.giants:DATA.fish),category,originalWeight:weight,weight,source:'Припасы ордена Мглистой Дымки',direct:false,removed:weight===0,tags:distorted?['<span class="island-negative-hint">Применено «Нестабильное присутствие»</span>']:[],debuffLimited:false,islandDistorted:distorted,islandOriginalWeight:distorted?natural:null,islandDistortedWeight:distorted?weight:null};if(weight===0){f.skeletonOf=f.name;f.name=`рыбный скелет (${f.name})`;f.islandSkeleton=f.mythicSkeleton=true;}state.fish.push(f);if(parentHistoryId)attachFishToHistoryRow(parentHistoryId,f.id);});if(pack.resource)state.tradeItems.push({id:uid(),key:pack.resource.key,name:pack.resource.name,icon:pack.resource.icon,exchanged:false,islandTrade:true,mistSupplies:true});});
...
function useRunicFlare(){ensureIslands();if(state.finished||state.islands.flares<=0||state.tradeShipArrived)return;const launch=source=>{state.islands.flares--;if(state.islands.destructiveFlares>0){state.islands.destructiveFlares--;addIslandFeat('stormSignal');}playSound('signalFlareLaunch');beginTradeShip(source);commitState();},hasRecyclableTrash=state.trash.some(item=>item.recyclonEligible&&!item.converted&&!item.recycled);if(!hasRecyclableTrash){launch('flare');return;}showChoice('Руническая сигнальная ракета','Какое судно призвать?',[{label:'Торговое судно',value:'trade'},{label:'«Рециклон» — переработать хлам',value:'recyclon'}],value=>launch(value==='recyclon'?'recyclon':'flare'));}
...
function castLine() {
  if (state.finished || state.castsLeft<=0 || $('choiceDialog').open || $('riftDialog')?.open || state.islands?.active || hasPendingAbyssalDecision()) return;
...
function renderTradeItemFind(row) {
  ...
  if(change?.kind==='transform')return `<div class="trade-item-find is-abyss-transformed result-${abyssChangeTargetType(change)}"><span class="trade-item-spark">${tradeItemIconMarkup(item)}</span><span class="abyss-object-source">Найден предмет: <strong>${item.name}</strong></span><span class="abyss-object-arrow">→</span>${renderAbyssChangeResult(change)}<small class="abyss-change-agent">${change.source}</small></div>`;
  ...
  return `<div class="trade-item-find${change?' is-abyss-removed':''}${actuallyTraded?' is-traded':''}"><span class="trade-item-spark">${tradeItemIconMarkup(item)}</span><span>Найден предмет: <strong>${item.name}</strong></span>${status}${notes}</div>`;
}
...
function createRecyclonFish(category,itemName,itemIcon=shipIconMarkup('recyclon','is-compact-icon')){
  const weight=category==='giant'?rand1(20,40):category==='heavy'?rand1(10,19.9):rand1(.1,9.9),name=pick(category==='giant'?DATA.giants:DATA.fish),fish={id:uid(),name,category,originalWeight:weight,weight,source:'Перерабатывающее судно «Рециклон»',direct:false,tradeFish:true,removed:false,tags:['получена от «Рециклона»','защищена от <span class="island-negative-hint">«Нестабильного присутствия»</span>'],debuffLimited:false,islandDistorted:false,islandOriginalWeight:null,islandDistortedWeight:null};state.fish.push(fish);state.tradeExchanges.push({id:uid(),itemId:uid(),itemKey:'recyclon',itemName,itemIcon,fishId:fish.id,fishName:fish.name,category,weight:fish.weight,tradeShipHistoryId:state.tradeShipHistoryId});return fish;
}
...
function renderRecyclonDialog(){
  $('tradeDialogTitle').innerHTML=`${shipIconMarkup('recyclon','is-dialog-icon')}<span>Перерабатывающее судно «Рециклон»</span>`;$('tradeDialogIntro').textContent='Судно перерабатывает хлам, полученный обычными забросами.';
  const trash=state.trash.filter(x=>x.recyclonEligible&&!x.converted&&!x.recycled),trashOffer=`<article class="trade-offer"><div class="trade-offer-icon">${shipIconMarkup('recyclon','is-offer-icon')}</div><div class="trade-offer-copy"><strong>Хлам обычных забросов</strong><span>Доступно: ${trash.length} • рыбы 50% / 35% / 15%</span></div><button type="button" data-recycle-trash ${!trash.length?'disabled':''}>Переработать всё</button></article>`,exchanges=(state.tradeExchanges||[]).filter(x=>x.tradeShipHistoryId===state.tradeShipHistoryId).map(x=>`<li><span>${x.itemIcon} ${x.itemName}</span><strong class="trade-history-result"><span class="trade-history-arrow">→</span>${tradeExchangeFishMarkup(x)}</strong></li>`).join('');$('tradeOffers').innerHTML=trashOffer;$('tradeResults').innerHTML=exchanges?`<h3>Результаты переработки</h3><ul>${exchanges}</ul>`:'';$('tradeAllBtn').textContent='Переработать весь хлам';$('tradeAllBtn').disabled=!trash.length||state.tradeShipCompleted;$('tradeFinishBtn').disabled=state.tradeShipCompleted;
}
function renderTradeDialog() {
  ...
  $('tradeDialogTitle').innerHTML=`${shipIconMarkup('trade','is-dialog-icon')}<span>Торговое судно</span>`;$('tradeDialogIntro').textContent='Команда судна готова обменять редкие находки на крупную рыбу.';$('tradeAllBtn').textContent='Обменять всё';
...
function renderHistory() {
  ...
  const visibleHistory=state.history.filter(row=>!row.mergedIntoHistoryId);
  ...
  $('historyList').innerHTML=visibleHistory.map((h)=>{
    ...
    const historyArtifactVisualName=h.type==='mythic'?(historyArtifact?artifactVisualName(historyArtifact):String(h.text).startsWith('Первобытный хаос')?'Первобытный хаос':h.text):h.text;
    ...
    const skeletonHistoryIcon=fish?fishSkeletonMainIcon(fish):'',eatenHistoryIcon=fish?fishEatenMainIcon(h,fish):'';
    const historyShipKind=h.type==='tradeShip'&&(h.tradeShipSource==='recyclon'||/Рециклон/i.test(String(h.text||'')))?'recyclon':'trade';
     const icon=h.type==='fish'&&h.islandDisplaced?'<span class="island-displaced-cross">✖</span>':h.type==='fish'&&fish?(skeletonHistoryIcon||eatenHistoryIcon||fishCategoryIcons(fish)):h.type==='coin'?coinIconMarkup(coinForRow?.type||'copper'):h.type==='weather'?weatherWarningIconMarkup('is-history-icon'):expeditionItem?expeditionItemIconMarkup(expeditionItem.name,'is-history-icon'):h.type==='tradeShip'?shipIconMarkup(historyShipKind,'is-history-icon'):(abyssEntityIcon||entityBasedIcon||icons[h.type]||'');
    ...
    const namedRiftIcon=fish&&!skeletonHistoryIcon&&!eatenHistoryIcon?(riftFishIcon(fish)||islandFishIconMarkup(fish)):'',namedRiftBadges=fish?(riftFishBadges(fish)||(namedRiftIcon&&islandFishKind(fish)?islandFishBadges(fish):'')):'';
...
const GUIDE = {
  ...
  'Острова': [
    ...
    ['Руническая сигнальная ракета','Одноразово гарантирует отдельный визит торгового судна даже во время Шторма. Только вызванное ракетой судно принимает исполинов. После обмена последнего исполина будущая рыба больше не искажается, но прежний улов не восстанавливается. В конце сессии всё равно выполняется обычная независимая проверка прибытия судна.'],
...
$('tradeOffers').addEventListener('click',event=>{
  const recycleButton=event.target.closest('[data-recycle-trash]');if(recycleButton){recycleAllTrash();return;}
