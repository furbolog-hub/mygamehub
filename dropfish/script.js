...
function tradeItemByKey(key) { return TRADE_ITEMS.find(item=>item.key===key); }
...
function initialState() {
  return {
    ...
    arcadeCaughtCount:0, arcadeLastSpawnCast:-1, pendingSeagulls:[], coins:[], pendingCoinBoosts:[], tradeItems:[], tradeShipChecked:false, tradeShipArrived:false, tradeShipCompleted:false, tradeShipOffers:[], tradeExchanges:[], tradeShipHistoryId:null, sessionDate:null, finalResult:null
  };
}
...
function addFishHistory(fish, source) {
  const giant=fish.category==='giant';
  return addHistory(`${giant?'🏆 ':''}${capitalize(fish.name)}${giant?' 💪🏼':''} — ${kg(fish.weight)}`,'fish',`${source}${fish.tags.length?` • ${fish.tags.join(' • ')}`:''}`,{fishId:fish.id,eaten:Boolean(fish.removed),arcade:Boolean(fish.arcadeCatch),numbered:source==='Заброс'});
}
...
function castLine() {
  ...
  if (chance(BALANCE.events.angusChance)) encounterAngus();
  if (chance(BALANCE.events.weatherChangeChance)) changeWeatherRandomly();
...
function finalFishSnapshot() {
  ...
  fish.forEach(f=>{ if(!f.tradeFish&&activeDebuff('Касатка')&&!state.megalodon&&f.weight>=5.5) f.removed=true; });
...
function renderTradeItemFind(row) {
  ...
  if (!item) return '';
  const status=item.exchanged?'<span class="trade-item-used">обменян</span>':'';
  return `<div class="trade-item-find"><span class="trade-item-spark">${item.icon}</span><span>Найден предмет: <strong>${item.name}</strong></span>${status}</div>`;
}
function renderTradeShipHistory(row) {
  if (row.type!=='tradeShip') return '';
  const exchanges=(state.tradeExchanges||[]).map(exchange=>`<li><span>${exchange.itemIcon} ${exchange.itemName}</span><span>→ ${exchange.category==='giant'?'🏆':'🐟'} ${capitalize(exchange.fishName)} — ${kg(exchange.weight)}</span></li>`).join('');
  const disabled=state.tradeShipCompleted?'disabled':'';
  return `<div class="trade-ship-history-copy">Команда судна готова обменять редкие находки на крупную рыбу.</div>
    ${exchanges?`<ul class="trade-history-results">${exchanges}</ul>`:''}
    <button type="button" class="trade-open-btn" data-open-trade ${disabled}>${state.tradeShipCompleted?'Торговля завершена':'Обмен'}</button>`;
}
function createTradeFish(item) {
  const category=chance(BALANCE.tradeShip.giantChance)?'giant':'heavy';
  const original=category==='giant'?rand1(20,40):rand1(10,19.9);
  const fish={id:uid(),name:pick(category==='giant'?DATA.giants:DATA.fish),category,originalWeight:original,weight:original,source:'Торговое судно',direct:false,tradeFish:true,removed:false,tags:['обмен с торговым судном'],debuffLimited:false};
  state.fish.push(fish);
  const exchange={id:uid(),itemId:item.id,itemKey:item.key,itemName:item.name,itemIcon:item.icon,fishId:fish.id,fishName:fish.name,category,weight:fish.weight};
  state.tradeExchanges.push(exchange);
  item.exchanged=true;
  return exchange;
}
function exchangeTradeItem(key) {
  if (state.tradeShipCompleted || !(state.tradeShipOffers||[]).includes(key)) return;
  const item=availableTradeItemsForKey(key)[0];
  if (!item) return;
  const exchange=createTradeFish(item);
  TelegramApp?.HapticFeedback?.notificationOccurred?.('success');
  showVisualEffect(exchange.category==='giant'?'giant':'bonus',exchange.category==='giant'?'🏆':'🐟',exchange.category==='giant'?'РЫБА-ГИГАНТ':'ТЯЖЕЛОВЕС',`${capitalize(exchange.fishName)} — ${kg(exchange.weight)}`,1000,true);
  renderTradeDialog(); render();
}
function exchangeAllTradeItems() {
  if (state.tradeShipCompleted) return;
  const offerSet=new Set(state.tradeShipOffers||[]);
  const items=(state.tradeItems||[]).filter(item=>!item.exchanged&&offerSet.has(item.key));
  if (!items.length) { toast('Нет подходящих предметов для обмена'); return; }
...
function renderTradeDialog() {
  ...
  const offers=(state.tradeShipOffers||[]).map(key=>{
    const item=tradeItemByKey(key); const count=counts[key]||0;
    return `<article class="trade-offer${count?'':' is-empty'}"><div class="trade-offer-icon">${item.icon}</div><div class="trade-offer-copy"><strong>${item.name}</strong><span>В наличии: ${count}</span></div><button type="button" data-trade-key="${key}" ${!count||state.tradeShipCompleted?'disabled':''}>Обменять 1</button></article>`;
  }).join('');
  const exchanges=(state.tradeExchanges||[]).map(exchange=>`<li><span>${exchange.itemIcon} ${exchange.itemName}</span><strong>→ ${exchange.category==='giant'?'🏆 гигант':'🐟 тяжеловес'}: ${capitalize(exchange.fishName)} — ${kg(exchange.weight)}</strong></li>`).join('');
  $('tradeOffers').innerHTML=offers||'<p class="muted">Предложения ещё не сформированы.</p>';
  $('tradeResults').innerHTML=exchanges?`<h3>Результаты обмена</h3><ul>${exchanges}</ul>`:'';
  const canExchange=(state.tradeItems||[]).some(item=>!item.exchanged&&(state.tradeShipOffers||[]).includes(item.key));
...
function completeTradeShip() {
  ...
  $('tradeDialog')?.close();
  addHistory('Торговля с судном завершена','event',`(Получено рыб: ${(state.tradeExchanges||[]).length})`);
  finishGame();
}
function beginTradeShip() {
  state.tradeShipArrived=true;
  state.tradeShipOffers=sampleUnique(TRADE_ITEMS,BALANCE.tradeShip.offerCount).map(item=>item.key);
  const row=addHistory('Торговое судно прибыло','tradeShip','',{numbered:false});
  state.tradeShipHistoryId=row.id;
  showVisualEffect('legendary','🚢','ТОРГОВОЕ СУДНО','Команда судна готова к обмену',1300);
  TelegramApp?.HapticFeedback?.notificationOccurred?.('success');
  toast('Торговое судно прибыло — нажмите «Обмен» в хронологии');
  render();
}
...
function render() {
  ...
  TRADE_ITEMS.forEach(item=>{ if (tradeCounts[item.key]) effects.push({label:`${item.icon} ${item.name} ×${tradeCounts[item.key]}`,kind:'trade'}); });
...
function renderHistory() {
  const icons={fish:'🐟',bonus:'✅',debuff:'🛑',epic:'💜',legendary:'🧡',trash:'🔘',coin:'🪙',weather:'⚠️',angus:'🧔',tradeShip:'🚢'};
  ...
  $('historyList').innerHTML=state.history.map((h)=>{
    ...
    return `<li data-history-id="${h.id}" class="history-item type-${h.type}${h.eaten?' is-eaten':''}${h.stolen?' is-stolen':''}${h.transmutation?' is-transmutation':''}${weatherClass}${h.id===lastAnimatedHistoryId?' is-new':''}"><strong>${mainLine}</strong>${h.detail?`<small class="history-detail">${h.detail}</small>`:''}${impactNote?`<small class="history-impact-note">${impactNote}</small>`:''}${essenceImpact}${goldenHourImpact}${leviathanImpact}${coinImpacts}${flipperImpact}${maskImpact}${scubaImpact}${gearBonusStatus}${scubaBonusStatus}${tradeItemFind}${tradeShipHistory}${coinAction}${transmutation}${embeddedFish}${rowAmbient}</li>`;
  }).join('');
...
function showChoice(title,text,options,onSelect) {
  ...
  options.forEach(option=>{const b=document.createElement('button');b.textContent=option;b.onclick=()=>{$('choiceDialog').close();onSelect(option);render();if(state.castsLeft<=0&&!state.finished)maybeFinalizeSession();};box.appendChild(b);});
...
const GUIDE = {
  ...
  'Торговое судно': [
    ['🚢 Прибытие','После последнего заброса в любую погоду, кроме Шторма, судно прибывает с вероятностью 50%. Пока торговля не завершена, итог сессии и достижения не рассчитываются.'],
    ['✨ Предметы обмена','При прямой поимке рыбы удочкой с вероятностью 40% дополнительно находится один специальный предмет. Предмет не расходует заброс и показывается в той же строке хронологии.'],
    ['🐟 Обмен','Команда судна принимает три случайных вида предметов из шести. За каждый предмет выдаётся тяжеловес с вероятностью 90% или рыба-гигант с вероятностью 10%. Можно обменивать по одному или всё сразу.']
  ],
  'Бонусы': [
...
function buildDropfishDebugPanel() {
  ...
  const groups=[
    ['Погода',Object.entries(DATA.weather).map(([key,value])=>button(`${value.icon} ${value.name}`,`weather:${key}`)).join('')],
    ['Улов',[button('🐟 Обычная рыба','fish:normal'),button('🐟 Тяжеловес','fish:heavy'),button('🏆 Рыба-гигант','fish:giant')].join('')],
    ['Ангус и судно',[button('🧔 Вызвать Ангуса','angus'),button('🚢 Вызвать торговое судно','trade')].join('')],
    ['Бонусы',DATA.bonuses.map(name=>button(`${entityIcon(name,'✅')} ${name}`,`bonus:${encodeURIComponent(name)}`)).join('')],
    ['Дебафы',DATA.debuffs.map(name=>button(`${entityIcon(name,'🛑')} ${name}`,`debuff:${encodeURIComponent(name)}`)).join('')],
    ['Эпические',DATA.epics.map(name=>button(`${entityIcon(name,'💜')} ${name}`,`epic:${encodeURIComponent(name)}`)).join('')],
    ['Легендарные',DATA.legendary.map(name=>button(`${entityIcon(name,'🧡')} ${name}`,`legendary:${encodeURIComponent(name)}`)).join('')],
    ['Сессия',[button('+1 заброс','casts:+1'),button('Остался 1 заброс','casts:1'),button('Завершить сессию','finish','danger'),button('Новая тестовая сессия','reset')].join('')]
  ];
  ...
  grid.addEventListener('click',event=>{
    ...
    if(action.startsWith('weather:')){
      ...
      state.castsLeft=0; state.tradeShipChecked=true; state.tradeShipCompleted=true; if(!state.finished) finishGame();
...
$('tradeOffers').addEventListener('click',event=>{ const button=event.target.closest('[data-trade-key]'); if(button) exchangeTradeItem(button.dataset.tradeKey); });
$('tradeAllBtn').addEventListener('click',exchangeAllTradeItems);
